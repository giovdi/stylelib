...
class NForm extends \DeployStudio\Style\Base\NForm {
	...
	static function inputBuild ($label, $required, $options, $outputArr) {
		...
		$label_output = '';
		if (!is_null($label)) {
			$label_output = '<label class="' . $formOptions['classLabel'] . ' control-label text-right">' . $outputArr['requiredLabel'] . $label . '</label>';
		}
		$build_field = '<div class="'.$formOptions['formConst']['form-group'].' row">
			' . $label_output . '
			...
			' . (strlen($options['description']) > 0 ? '<div class="help-block"><small class="text-muted">' . $options['description'] . '</small></div>' : '') . '
			</div>
	...
	static function textareaBuild ($label, $required, $options, $outputArr) {
		...
		echo '<div class="'.$formOptions['formConst']['form-group'].'">
			<label class="' . $formOptions['classLabel'] . ' control-label text-right">' . $outputArr['requiredLabel'] . $label . '</label>
		
			<div class="' . $formOptions['classInput'] . ' row">
			...
			' . (isset($options['description']) && strlen($options['description']) > 0 ? '<div class="help-block"><small class="text-muted">' . $options['description'] . '</small></div>' : '') . '
	...
	static function checkboxesBuild ($label, $checkboxTags, $options, $outputArr) {
		...
		echo '<div class="'.$formOptions['formConst']['form-group'].'">
			...
			' . (!empty($options['description']) ? '<div class="help-block"><small class="text-muted">' . $options['description'] . '</small></div>' : '') . '
